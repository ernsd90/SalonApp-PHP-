...
$(document).ready(function() {
    ...
    $(document).on("change", "#jobcardform #service_staff0", function() {
    ...
    $(document).on("change keyup", "#jobcardform .service_price,#jobcardform .service_gst,#jobcardform .discount_mode,#jobcardform .discount,#jobcardform .extra_tax", function() {
        calcTotals_jobcard();
    });
    $(document).on("change keyup", "#jobcardform .part_cc", function() {
        var mytotal = calcTotals_jobcard();
        var part_cc = $("#jobcardform .part_cc").val();
       
        if (part_cc > mytotal) {
            toastr.error("Amount should be equal or less then total value", 'Service');
            $("#jobcardform .part_cc").val(mytotal);
            $("#jobcardform .part_cash").val('0');
        } else {
            
            var part_cash = mytotal-part_cc;
            $("#jobcardform .part_cash").val(part_cash);
        }
    });

    $(document).on("change keyup", "#jobcardform .part_cash", function() {
        var mytotal = calcTotals_jobcard();
    ...
    $(document).on("change", "#jobcardform .payment_mode", function() {
    ...
    $(document).on("change", "#jobcardform .sub_service", function() {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (obj.error == 1) {
                    ...
                    //console.log("Service Priice: "+new_price);
                    //console.log(current_tr.find(".service_price"));
                    calcTotals_jobcard();
                }
            },
            error: function() {
