...
function get_salerecord(){
    ...
    if($fromdate != '' && $todate == ''){
        ...
        // if($fromdate_month <= 03 || $fromdate_year < 2025) {
    ...
    if($fromdate != '' && $todate != ''){
        // Check if either date is before the cutoff

        $todate_month = date("m",strtotime($todate));
        $todate_year = date("Y",strtotime($todate));

        if($todate_month > 03 || $todate_year < 2025) {
            $fromdate = '2025-04-01';
        }
