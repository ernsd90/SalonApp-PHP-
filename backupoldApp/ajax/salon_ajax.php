...
function create_salon(){
    ...
    if($ttl > 0){
        ...
        $error = 1;
         ...
         $sql = "INSERT INTO `hr_salon` SET `salon_name`='".$salon_name."',`salon_address`='".$salon_address."',`salon_contact`='".$salon_contact."',`gst_enable`='".$gst_enable."',`salon_gst`='".$salon_gst."',`include_gst`='".$include_gst."',`firm_name`='".$firm_name."',`whatsapp_enable`='".$whatsapp_enable."',`whatsapp_api`='".$whatsapp_api."'";
        insert_query($sql);
...
function update_salon(){
	...
	$sql = "UPDATE `hr_salon` SET `salon_name`='".$salon_name."',`salon_address`='".$salon_address."',`salon_contact`='".$salon_contact."',`gst_enable`='".$gst_enable."',`salon_gst`='".$salon_gst."',`include_gst`='".$include_gst."',`firm_name`='".$firm_name."',`whatsapp_enable`='".$whatsapp_enable."',`whatsapp_api`='".$whatsapp_api."' Where salon_id = '".$salon_id."'";
    update_query($sql);
...
function delete_salon(){
	...
	$sql = "DELETE FROM `hr_salon` WHERE salon_id  = '".$id."' ";
    if(update_query($sql)){
		$msg = "Salon Delete Successfully";
	}else{
...
function get_salon(){
    ...
    if($salon){
        foreach($salon as $salons){
            ...
            $edit_btn = '<button type="button" class="btn btn-xs btn-outline-info modalButtonCommon" data-toggle="modal" data-href="salon_edit.php?salon_id='.$salon_id.'"><i class="fa fa-edit "></i></button>';
            $del_btn = '<button type="button" class="btn btn-xs btn-outline-danger modalButtonCommon" data-toggle="modal" data-href="salon_delete.php?salon_id='.$salon_id.'"><i class="fa fa-trash "></i> </button>';

            $status_btn = '<label class="badge badge-success">Active</label>';

            $salondata[$i] = $salons;
            $salondata[$i]['status'] = $status_btn;
            $salondata[$i]['action'] = $edit_btn.$del_btn;
