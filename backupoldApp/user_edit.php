    ...
    $session_user_type = isset($user_type) ? $user_type : null;
    $session_salon_id = isset($salon_id) ? $salon_id : null;
    ...
    if(isset($_REQUEST['user_id']) && is_numeric($_REQUEST['user_id'])){
        ...
        $sql = "SELECT salon_id as salon_ids,salon_name,salon_address FROM `hr_salon` " . ($session_user_type == 1 ? "" : "WHERE salon_id='".$session_salon_id."'");
						 ...
						 <div class=" card">
                            ...
                            <div class=" row">
                                ...
                                <div class="form-group ">
                                    ...
                                    <div class="">
                                            <select required name="salon_id" id="role" class="select2 form-control custom-select" style="width: 100%; height:45px;" <?php echo ($session_user_type != 1) ? 'disabled' : ''; ?>>
                                            <?php 	foreach($all_salon as $salon){
                                                foreach ($salon as $var => $sale){
                                                    $$var = $sale;
                                                }
                                            ?>
                                                <option <?php echo (isset($salon_id) && $salon_id == $salon_ids ? "selected":"");?> value="<?php echo $salon_ids;?>" ><?php echo $salon_name."(".$salon_address.")"; ?></option>
                                ...
                                <div class="form-group ">
									...
									<?php 	foreach($roles as $role){
											extract($role);
									?>
                                        <option <?php echo (isset($user_role_id) && $user_role_id == $role_id ? "selected":"");?> value="<?php echo $role_id;?>" ><?php echo $role_name; ?></option>
                            ...
                            <div class="col-md-6">
                                <div class="form-group ">
                                    <label class="control-label col-form-label">Admin Level</label>
                                    <div class="">
                                        <select required name="user_type" class="form-control custom-select">
                                            <option value="3" <?php echo (isset($edit_user_type) && $edit_user_type == 3 ? "selected" : "");?>>Staff User</option>
                                            <option value="2" <?php echo (isset($edit_user_type) && $edit_user_type == 2 ? "selected" : "");?>>Outlet Admin</option>
                                            <option value="1" <?php echo (isset($edit_user_type) && $edit_user_type == 1 ? "selected" : "");?>>Superadmin</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
