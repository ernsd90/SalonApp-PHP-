...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.12/css/select2.min.css">
    ...
    var salon_id = <?php echo $salon_id; ?>;
    var tax_value = <?php echo $tax_value; ?>;
    </script>
      ...
      <nav class="navbar default-layout-navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
        ...
        <div class="navbar-menu-wrapper d-flex align-items-stretch">
          ...
          <ul class="navbar-nav navbar-nav-right">
            <?php if(isset($userdata['user_type']) && $userdata['user_type'] == 1){ 
                $all_salons = select_array("SELECT salon_id, salon_name FROM hr_salon");
            ?>
            <li class="nav-item">
                <select class="form-control" style="min-width: 200px;" id="superadmin_salon_switcher" onchange="switchSuperadminSalon(this.value)">
                    <?php if($all_salons) foreach($all_salons as $s){ ?>
                        <option value="<?php echo $s['salon_id']; ?>" <?php if($salon_id == $s['salon_id']) echo 'selected'; ?>><?php echo $s['salon_name']; ?></option>
                    <?php } ?>
                </select>
                <script>
                function switchSuperadminSalon(val){
                    var xhr = new XMLHttpRequest();
                    xhr.open("POST", "ajax/user_ajax.php", true);
                    ...
                    xhr.send("method=switch_salon&new_salon_id=" + val);
                }
                </script>
            </li>
            <?php } ?>
     ...
     <?php  if(isset($userdata['user_type']) && $userdata['user_type'] == 1){ ?>
        ...
        <div class="collapse" id="ui-user">
          <ul class="nav flex-column sub-menu">
            ...
            <li class="nav-item"> <a class="nav-link" href="salons.php">Manage Outlets</a></li>
          </ul>
        </div>
      </li>
     <?php } ?>
