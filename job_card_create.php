...
if (isset($_POST['cust_mob'])) {
    ...
    if (isset($_POST['job_card_id']) && is_numeric($_POST['job_card_id'])) {
        ...
        if ($job_card['job_card_id'] > 0) {
            ...
            echo json_encode(['error' => 0, 'msg' => "Job card updated successfully", 'job_card_id' => $job_card_id]);
        } else {
            echo json_encode(['error' => 1, 'msg' => "Job card not found."]);
        }
    } else {
        ...
        if ($job_card_id > 0) {
            ...
            echo json_encode(['error' => 0, 'msg' => "Job card created successfully", 'job_card_id' => $job_card_id]);
        } else {
            echo json_encode(['error' => 1, 'msg' => "Failed to create job card."]);
        }
    }
}
exit;
