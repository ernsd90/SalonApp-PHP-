...
try {
    $conn->query("ALTER TABLE `hr_salon` ADD COLUMN `make_enable` TINYINT(1) NOT NULL DEFAULT 0 AFTER `whatsapp_enable`");
} catch(Exception $e) {
    // Column already exists, ignore
}
try {
    $conn->query("ALTER TABLE `hr_salon` ADD COLUMN `make_webhook_url` VARCHAR(500) NOT NULL DEFAULT '' AFTER `make_enable`");
} catch(Exception $e) {
    // Column already exists, ignore
}
try {
...
try {
    $conn->query("ALTER TABLE `hr_salon` ADD COLUMN `google_review_link` VARCHAR(1000) NOT NULL DEFAULT ''");
} catch(Exception $e) {
    // Column already exists, ignore
}
...
function create_salon(){
    $error = 0; 
    extract($_POST);

    $sql = "SELECT * FROM `hr_salon` WHERE `salon_name`='".$salon_name."'";
    $ttl = num_rows($sql);
    if($ttl > 0){
        $msg = "Salon Already Exist";
        $error = 1;
    }else{
        $gst_percentage = floatval($gst_percentage);
        $make_enable = isset($make_enable) ? intval($make_enable) : 0;
...
function delete_salon(){
    $error = 0;
    extract($_POST);

	$sql = "UPDATE `hr_salon` SET salon_status = 0 WHERE salon_id  = '".$id."' ";
    if(update_query($sql)){
		$msg = "Salon Deactivated Successfully";
	}else{
		$msg = "Error While Deactivate";
		$error = 1;
	}
    return array("msg" => $msg,"error"=>$error);
}

function toggle_salon_status(){
    $error = 0;
    extract($_POST);
    $status = intval($status);
    $sql = "UPDATE `hr_salon` SET salon_status = '$status' WHERE salon_id = '$salon_id'";
    update_query($sql);
    $msg = $status == 1 ? "Salon Activated" : "Salon Deactivated";
    return array("msg" => $msg, "error" => $error);
}
...
function service_price(){
    ...
    } elseif (strpos($service_id, 's_') === 0) {
        $id = str_replace('s_', '', $service_id);
        $sql = "SELECT service_price FROM `hr_services` WHERE service_id='".$id."'";
    } else {
        $sql = "SELECT service_price FROM `hr_services` WHERE service_id='".$service_id."'";
    }
    $service = select_row($sql);
    return $service['service_price'] ?? 0;
}
...
function get_salon(){
    ...
    if(isset($search['value']) && $search['value'] != ''){
        ...
        $where = " WHERE salon_name LIKE '%".$search_value."%' OR salon_address LIKE '%".$search_value."%' OR salon_contact LIKE '%".$search_value."%'";
    }

    if (isset($start)) { $page  = $start; } else { $page=1; }; 
    $start_from = $start; 

    $sql = "SELECT * FROM `hr_salon` $where ORDER BY salon_id desc";
    $total_records = num_rows($sql); 
    $sql .= " LIMIT $start_from, $length";
    $salon = select_array($sql);
    $salondata = array();
    $i=0;
    if($salon){
        foreach($salon as $salons){
            ...
            $edit_btn = '<button type="button" class="btn-edit modalButtonCommon" data-toggle="modal" data-href="salon_edit.php?salon_id='.$salon_id.'"><i class="ph ph-pencil-simple"></i> Edit</button>';
            
            if($salon_status == 1) {
                $toggle_btn = '<button type="button" class="btn-deactivate" onclick="toggleSalonStatus('.$salon_id.', 0)"><i class="ph ph-x-circle"></i> Deactivate</button>';
            } else {
                $toggle_btn = '<button type="button" class="btn-activate" onclick="toggleSalonStatus('.$salon_id.', 1)"><i class="ph ph-check-circle"></i> Activate</button>';
            }

            $salondata[$i] = $salons;
            $salondata[$i]['salon_mobile'] = $salon_contact; // alias for Datatables
            $salondata[$i]['action'] = '<div style="display:flex; gap:8px;">' . $edit_btn . $toggle_btn . '</div>';
...
function update_services_cat(){
    ...
    return array("msg" => $msg,"error"=>($error ?? 0));
}
...
function create_services(){
    ...
    $service_status = isset($service_status) ? 1 : 0;
    $service_reminder = isset($service_reminder) ? $service_reminder : '0';
    ...
    if($ttl > 0){
        ...
        $service_name = ucwords(strtolower(trim($service_name)));
         $sql = "INSERT INTO `hr_services` SET `service_name`='".$service_name."',`salon_id`='".$salon_id."',`service_price`='".$service_price."',`service_status`='".$service_status."',`service_reminder`='".$service_reminder."', `service_catid`='".$service_catid."',`user_id`='".$user_id."'";
        insert_query($sql);
...
function update_services(){
    ...
    $service_name = ucwords(strtolower(trim($service_name)));
    $sql = "UPDATE `hr_services` SET `service_name`='".$service_name."', `service_price`='".$service_price."',`service_status`='".$service_status."',`service_catid`='".$service_catid."',`service_reminder`='".$service_reminder."', `salon_id`='".$salon_id."' Where service_id = '".$service_id."'";
...
function get_service(){
    ...
    $userdata = array();
        ...
        foreach($user as $users){
            extract($users);
            // if(check_user_permission("cataloge","edit",$user_id)){
                $edit_btn = '<button type="button" class="btn-edit modalButtonCommon" data-toggle="modal" data-href="services_edit.php?service_id='.$service_id.'"><i class="ph ph-pencil-simple"></i> Edit</button>';
            // }

            if($users['service_status'] == 1) {
                $toggle_btn = '<button type="button" class="btn-deactivate" onclick="toggleServiceStatus('.$service_id.', 0)"><i class="ph ph-x-circle"></i> Deactivate</button>';
            } else {
                $toggle_btn = '<button type="button" class="btn-activate" onclick="toggleServiceStatus('.$service_id.', 1)"><i class="ph ph-check-circle"></i> Activate</button>';
            }
...
function toggle_service_status(){
    $error = 0;
    extract($_POST);
    $status = intval($status);
    $sql = "UPDATE `hr_services` SET service_status = '$status' WHERE service_id = '$service_id'";
    update_query($sql);
...
function create_product_brand(){
    ...
    $sql = "SELECT brand_id FROM `hr_product_brand` WHERE `brand_name`='".$brand_name."' and `salon_id`='".$salon_id."'"; 
     $ttl = num_rows($sql);
    if($ttl > 0){
        ...
        $status_val = isset($status) ? 1 : 0;
         $sql = "INSERT INTO `hr_product_brand` SET `salon_id`='".$salon_id."', `user_id`='".$user_id."', `brand_name`='".$brand_name."', `brand_status` = '".$status_val."'";
        insert_query($sql);
...
function update_product_brand(){
    ...
    $brand_name = ucwords(strtolower(trim($brand_name)));
    $status_val = isset($status) ? 1 : 0;
	 $sql = "UPDATE `hr_product_brand` SET `brand_name`='".$brand_name."',`brand_status`='".$status_val."'  Where brand_id = '".$brand_id."'";
...
function get_product_brand(){
    ...
    $userdata = array();
        ...
        foreach($user as $users){
            ...
            if(check_user_permission("cataloge","edit",$user_id)){
                $edit_btn = '<button type="button" class="modalButtonCommon" data-href="product_brand_edit.php?brand_id='.$brand_id.'" style="background:#e0e7ff; color:#4f46e5; border:none; width:32px; height:32px; border-radius:8px; cursor:pointer; font-size:15px; display:inline-flex; align-items:center; justify-content:center; margin-right:6px;" title="Edit"><i class="ph ph-pencil-simple"></i></button>';
            } 
			 if(check_user_permission("cataloge","delete",$user_id)){
                $del_btn = '<button type="button" class="modalButtonCommon" data-href="common_delete.php?brand_id='.$brand_id.'" style="background:#fee2e2; color:#dc2626; border:none; width:32px; height:32px; border-radius:8px; cursor:pointer; font-size:15px; display:inline-flex; align-items:center; justify-content:center;" title="Delete"><i class="ph ph-trash"></i></button>';
            }

            $userdata[$i] = $users;
            // Native brand_status unchanged for JS
            $userdata[$i]['action'] = $edit_btn.$del_btn;
   ...
   if($search['value'] != ''){
    ...
    foreach($user as $users){
        ...
        if(check_user_permission("product","edit",$user_id)){
            $edit_btn = '<button type="button" class="modalButtonCommon" data-href="product_edit.php?product_id='.$product_id.'" style="background:#e0e7ff; color:#4f46e5; border:none; width:32px; height:32px; border-radius:8px; cursor:pointer; font-size:15px; display:inline-flex; align-items:center; justify-content:center; margin-right:6px;" title="Edit"><i class="ph ph-pencil-simple"></i></button>';
        } 
         if(check_user_permission("product","delete",$user_id)){
            $del_btn = '<button type="button" class="modalButtonCommon" data-href="common_delete.php?product_id='.$product_id.'" style="background:#fee2e2; color:#dc2626; border:none; width:32px; height:32px; border-radius:8px; cursor:pointer; font-size:15px; display:inline-flex; align-items:center; justify-content:center;" title="Delete"><i class="ph ph-trash"></i></button>';
        }
        $status_btn = '<label class="badge badge-danger">Inactive</label>';
...
function create_expenses(){
    ...
    $exp_date_val = isset($exp_date) && !empty($exp_date) ? $exp_date : date('Y-m-d');
  
         $sql = "INSERT INTO `hr_expenses` SET `exp_name`='".$exp_name."',`salon_id`='".$salon_id."',`payment_mode`='".$payment_mode."',`exp_vendor`='".$exp_vendor."',`exp_total`='".$exp_total."',`exp_note`='".$exp_note."', `exp_catId`='".$exp_catId."',`user_id`='".$user_id."', `exp_date`='".$exp_date_val."'";
        insert_query($sql);
...
function expenses_summary(){
    ...
    if(isset($fromdate) && $fromdate != '' && isset($todate) && $todate != ''){
        ...
        $date_where = " and DATE(e.exp_date) BETWEEN '".$fd."' AND '".$td."'";
    } else if(isset($fromdate) && $fromdate != '') {
        $fd = date("Y-m-d", strtotime($fromdate));
        $date_where = " and DATE(e.exp_date) = '".$fd."'";
    }

    if(isset($category_id) && $category_id != '') {
        $where .= " and e.exp_catId='".intval($category_id)."'";
    }

    $base_sql = "FROM `hr_expenses` AS e LEFT JOIN `hr_expenses_category` AS c ON c.exp_catId = e.exp_catId WHERE e.`salon_id`='".$salon_id."' ".$where." ".$date_where;

    // Overall totals
    $overall = select_row("SELECT COUNT(*) as cnt, COALESCE(SUM(e.exp_total),0) as grand_total ".$base_sql);
    $data['exp_number'] = intval($overall['cnt']);
    $data['exp_total'] = number_format(floatval($overall['grand_total']), 2);

    // Per-payment-method breakdown
    $breakdown = select_array("SELECT e.payment_mode, COUNT(*) as cnt, COALESCE(SUM(e.exp_total),0) as total ".$base_sql." GROUP BY e.payment_mode ORDER BY total DESC");
...
function get_expenses(){
    ...
    extract($_REQUEST);
        ...
        // Search filter
        if(isset($search['value']) && $search['value'] != ''){
            $search_value = mysqli_real_escape_string($GLOBALS['conn'], $search['value']);
            $where .= " and (e.exp_name LIKE '%".$search_value."%' OR e.exp_note LIKE '%".$search_value."%' OR e.exp_vendor LIKE '%".$search_value."%' OR c.category_name LIKE '%".$search_value."%' OR e.payment_mode LIKE '%".$search_value."%')";
        }

        // Date filter
        if(isset($fromdate) && $fromdate != '' && (!isset($todate) || $todate == '')){
            $fd = date("Y-m-d", strtotime($fromdate));
            $where .= " and DATE(e.exp_date) = '".$fd."'";
        }
        if(isset($fromdate) && $fromdate != '' && isset($todate) && $todate != ''){
            $fd = date("Y-m-d", strtotime($fromdate));
            $td = date("Y-m-d", strtotime($todate));
            $where .= " and DATE(e.exp_date) BETWEEN '".$fd."' AND '".$td."'";
        }
        ...
        // Payment mode filter — JS sends it as 'expence_type', only filter if non-empty
        $pm_filter = isset($expence_type) ? trim($expence_type) : '';
        if($pm_filter != '' && $pm_filter != 'all') {
            $where .= " and e.payment_mode='".mysqli_real_escape_string($GLOBALS['conn'], $pm_filter)."'";
        }

        $order_by = "e.exp_id DESC";
        if(isset($order[0]['column'])) {
            if($order[0]['column'] == 3){ $order_by = "e.exp_total ".$order[0]['dir']; }
            if($order[0]['column'] == 5){ $order_by = "e.exp_date ".$order[0]['dir']; }
        }

        $start_from = isset($start) ? intval($start) : 0;

        // LEFT JOIN so expenses without a category still appear
        $sql = "SELECT e.*, c.category_name FROM `hr_expenses` AS e LEFT JOIN `hr_expenses_category` AS c ON c.exp_catId = e.exp_catId WHERE e.`salon_id`='".$salon_id."' ".$where." ORDER BY ".$order_by;
        $total_records = num_rows($sql);
        $sql .= " LIMIT $start_from, $length";
    ...
    $userdata = array();
        ...
        foreach($user as $users){
            ...
            if(check_user_permission("expenses","edit",$loginuser_id)){
                $edit_btn = '<button type="button" class="modalButtonCommon" data-href="expenses_edit.php?exp_id='.$exp_id.'" style="background:#e0e7ff; color:#4f46e5; border:none; width:32px; height:32px; border-radius:8px; cursor:pointer; font-size:15px;"><i class="ph ph-pencil"></i></button>';
            }
            if(check_user_permission("expenses","delete",$loginuser_id)){
                $del_btn = '<button type="button" class="modalButtonCommon" data-href="common_delete.php?exp_id='.$exp_id.'" style="background:#fee2e2; color:#dc2626; border:none; width:32px; height:32px; border-radius:8px; cursor:pointer; font-size:15px;"><i class="ph ph-trash"></i></button>';
            }

            $userdata[$i] = $users;
            $userdata[$i]['exp_note'] = $users['exp_note'];
            $userdata[$i]['exp_date'] = date("d M Y", strtotime($exp_date));
            $userdata[$i]['category_name'] = $category_name ?: 'General';
            $userdata[$i]['payment_mode'] = $users['payment_mode'] ?? '';
            $userdata[$i]['action'] = '<div style="display:flex;gap:6px;">'.$edit_btn.$del_btn.'</div>';
...
function get_job_cards(){
    ...
    foreach($user as $users){
        extract($users);
        $action_btns = '<div style="display:flex; gap:8px;">';

        if($jobcard_status !== '2') {
            $action_btns .= '<a class="btn-edit" href="job_card.php?job_card_id='.$job_card_id.'"><i class="ph ph-pencil-simple"></i> Edit Card</a>';
            $action_btns .= '<a class="btn-activate" href="billing.php?job_card_id='.$job_card_id.'"><i class="ph ph-receipt"></i> Generate Bill</a>';
        } else {
            $action_btns .= '<span class="badge" style="background:#e2e8f0; color:#475569; padding:6px 12px; border-radius:6px; font-weight:600; font-size:13px; display:inline-flex; align-items:center;"><i class="ph ph-check-circle" style="margin-right:4px;"></i> Completed</span>';
        }
        
        $action_btns .= '<a class="btn-outline-secondary" href="print_jobcard.php?job_card_id='.$job_card_id.'" target="_blank" style="padding: 6px 12px; border-radius: 6px; border: 1px solid var(--border-color); color: var(--text-main); font-weight: 600; font-size: 13px; text-decoration: none; display:inline-flex; align-items:center;"><i class="ph ph-printer" style="margin-right:4px;"></i> Print</a>';
        
        $action_btns .= '</div>';

        $status_badge = $jobcard_status === '2' 
            ? '<span class="badge badge-success" style="background:#dcfce7; color:#16a34a; padding:4px 8px; border-radius:4px; font-size:12px; font-weight:600;">Billed</span>' 
            : '<span class="badge badge-warning" style="background:#fef3c7; color:#d97706; padding:4px 8px; border-radius:4px; font-size:12px; font-weight:600;">In Progress</span>';

        $userdata[$i] = $users;
        $userdata[$i]['created_date'] = date("d M Y, h:i A", strtotime($created_at));
        $userdata[$i]['jobcard_status'] = $status_badge;
        $userdata[$i]['action'] = $action_btns;
