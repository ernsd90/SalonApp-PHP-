...
$searchStart = '<div class="metrics-grid"';
$searchEnd = '</div>

<style>';

$startPos = strpos($content, $searchStart);
$endPos = strpos($content, $searchEnd, $startPos);

if($startPos !== false && $endPos !== false) {
    $pre = substr($content, 0, $startPos);
    $post = substr($content, $endPos + 6); // past </div>
...
EOT;
    ...
    file_put_contents($file, $pre . $newTiles . $post);
    echo "Replaced properly.";
} else {
    echo "Could not parse tiles boundary boundaries.";
}
?>
