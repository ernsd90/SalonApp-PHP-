...
<div class="dashboard-header" style="margin-bottom: 24px;">
    <h1 style="font-size: 24px; font-weight: 700; color: var(--text-main); margin-bottom: 4px;">Product Inventory Catalog</h1>
    <p style="color: var(--text-muted); font-size: 14px;">Manage the physical goods you sell, track brands, and set retail prices.</p>
</div>
...
<div class="card-modern" style="background: white; border-radius: var(--border-radius); border: 1px solid var(--border-color); box-shadow: var(--shadow-sm); overflow: hidden; margin-bottom: 30px;">
    ...
    <div style="padding: 20px 24px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
        <h3 style="font-size: 16px; font-weight: 600; margin: 0; color: var(--text-main);">Product List</h3>
        <div style="display: flex; gap: 12px;">
            <a href="product_brand.php" class="btn-primary" style="background: white; color: var(--text-main); border: 1px solid var(--border-color); width: auto; padding: 10px 16px; margin: 0; font-size: 14px; display: flex; align-items: center; gap: 8px; box-shadow: none;">
                <i class="ph-bold ph-tag"></i> Manage Brands
            </a>
            <button class="btn-primary" style="width: auto; padding: 10px 16px; margin: 0; font-size: 14px; display: flex; align-items: center; gap: 8px;" onclick="loadModal('product_edit.php');">
    ...
    <div style="padding: 24px;">
        <div class="table-responsive">
            <table id="get_product" class="table-modern" style="width: 100%;">
                <thead>
                    <tr>
        ...
        "columns": [
            ...
            { 
                ...
                "render": function(data) {
                    return '<span style="color: var(--text-muted); font-size: 13px; text-transform: uppercase;">' + (data ? data : 'No Brand') + '</span>';
                }
            },
            { 
                "data": "product_name",
                "render": function(data, type, row) {
                    return '<div style="font-weight:600;"><i class="ph ph-bag" style="color:var(--text-muted); font-size: 18px; vertical-align:middle; margin-right:6px;"></i> ' + data + '</div>';
                }
            },
            { 
                "data": "product_price",
        ...
        success: function(res) {
            try {
                ...
                if (obj.error == 1) {
                    ...
                    if ($.fn.DataTable.isDataTable('#get_product')) $('#get_product').DataTable().draw(false);
                }
            } catch(e) {
